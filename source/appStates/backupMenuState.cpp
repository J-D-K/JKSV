...
void backupMenuState::update(void)
{
    ...
    if (sys::input::buttonDown(HidNpadButton_A) && selected == 0)
    {
        ...
        if (m_SaveListing->getListingCount() > 0)
        {
            ...
            sharedAppState newBackupState = createNewProgressTask(createNewBackup, backupTaskData);
            // Push it
            jksv::pushState(newBackupState);
        }
        else
        {
    ...
    else if (sys::input::buttonDown(HidNpadButton_A) && selected > 0 && m_SaveListing->getListingCount() > 0)
    {
        ...
        // Create confirmation
        confirmAndPerformAction(confirmOverwrite, overwriteBackup, paths, sys::taskTypes::TASK_TYPE_PROGRESS);
    }
    else if (sys::input::buttonDown(HidNpadButton_Y) && selected > 0)
    {
        ...
        // Create confirmation
        confirmAndPerformAction(confirmRestore, restoreBackup, paths, sys::taskTypes::TASK_TYPE_PROGRESS);
    }
    else if (sys::input::buttonDown(HidNpadButton_X) && selected > 0)
    {
        ...
        confirmAndPerformAction(confirmDelete, deleteBackup, paths, sys::taskTypes::TASK_TYPE_TASK);
    }
