...
void titleSelectionState::update(void)
{
    ...
    if (sys::input::buttonDown(HidNpadButton_A))
    {
        ...
        if (fs::mountSaveData(currentUserSaveInfo->getSaveDataInfo()))
        {
            ...
            sharedAppState backupState = createBackupMenuState(m_CurrentUser, currentUserSaveInfo, currentTitleInfo);
            jksv::pushState(backupState);
        }
        else
        {
    ...
    else if (sys::input::buttonDown(HidNpadButton_X))
    {
        ...
        sharedAppState optionState = createTitleOptionState(m_CurrentUser, currentUserSaveInfo, currentTitleInfo);
        jksv::pushState(optionState);
    }
