...
static void loadTranslationFile(const std::string &filePath)
{
    fs::fileParser translationFile(filePath);

    while(translationFile.readLine())
    {
        // This is the "name" of the string
        std::string stringName = translationFile.getCurrentItemName();
        // This is the index of the string to use. This is mostly for menus.
        int stringIndex = translationFile.getNextValueAsInt();
        // This is the actual string JKSV will use.
        std::string uiString = translationFile.getNextValueAsString();
        // Add it to map.
        addUIString(stringName, stringIndex, uiString);
    }
}

// Initializes default US English strings.
static void initStringsToENUS(void)
{
    // Init strings
...
{
    // Get system language.
    SetLanguage systemLanguage = static_cast<SetLanguage>(0);
    uint64_t languageCode = 0;
    Result getLanguageCode = setGetLanguageCode(&languageCode);
    Result makeLanguageCode = setMakeLanguage(languageCode, &systemLanguage);
    // If either failed, or language is US...
    if(R_FAILED(getLanguageCode) || R_FAILED(makeLanguageCode) || systemLanguage == SetLanguage_ENUS || systemLanguage == SetLanguage_ENGB)
    {
        // Init default
        initStringsToENUS();
    }
    ...
    {
        // Everything went as it should, determine path.
        std::string translationFilePath = "romfs:/Translations/";
        switch(systemLanguage)
        {
            case SetLanguage_JA:
            {
                translationFilePath += "ja.txt";
            }
            break;
            // Skip US/UK
            case SetLanguage_FR:
            {
                translationFilePath += "fr.txt";
            }
            break;

            case SetLanguage_DE:
            {
                translationFilePath += "de.txt";
            }
            break;

            case SetLanguage_IT:
            {
                translationFilePath += "it.txt";
            }
            break;

            case SetLanguage_ES:
            {
                translationFilePath += "es.txt";
            }
            break;
            ...
            {
                translationFilePath += "zh-CN.txt";
            }
            break;

            case SetLanguage_KO:
            {
                translationFilePath += "ko.txt";
            }
            break;

            case SetLanguage_NL:
            {
                translationFilePath += "nl.txt";
            }
            break;

            case SetLanguage_PT:
            {
                translationFilePath += "pt.txt";
            }
            break;

            case SetLanguage_RU:
            {
                translationFilePath += "ru.txt";
            }
            break;
            ...
            {
                translationFilePath += "zh-TW.txt";
            }
            break;

            case SetLanguage_FRCA:
            {
                translationFilePath += "fr-CA.txt";
            }
            break;

            case SetLanguage_ES419:
            {
                translationFilePath += "es-419.txt";
            }
            break;

            // I have trouble testing/seeing this one?
            case SetLanguage_PTBR:
            {
                translationFilePath += "pt-BR.txt";
            }
            break;

            // Just pray and use this.
            default:
            {
                translationFilePath += "en-US.txt";
            }
            break;
        }
        // That was long. Load it now.
        loadTranslationFile(translationFilePath);
    }

    // Should be init'd/loaded now...
