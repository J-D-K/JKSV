...
namespace drive
{
    class gd : public rfs::IRemoteFS
    {
        public:
            ...
            void setRefreshToken(const std::string &_refreshToken)
            {
                rToken = _refreshToken;
            }
            ...
            size_t getDriveListCount() const
            {
                return driveList.size();
            }
            rfs::RfsItem *getItemAt(unsigned int _ind)
            {
                return &driveList[_ind];
            }
