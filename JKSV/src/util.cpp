...
void util::copyDirListToMenu(const FsLib::Directory &d, ui::menu &m)
{
    m.reset();
    m.addOpt(NULL, ".");
    m.addOpt(NULL, "..");
    ...
    {
        if (d.EntryAtIsDirectory(i))
        {
            m.addOpt(NULL, "D " + d.GetEntryNameAt(i));
        }
        else
        {
            m.addOpt(NULL, "F " + d.GetEntryNameAt(i));
        }
    }
}
